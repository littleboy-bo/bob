    ...
    static {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Load [error] folder error code configuration file [{}] under CLASSPATH", jarResources.length);
            }
            for (Resource resource : jarResources) {
                String fileName = resource.getFilename();
                fileName = fileName.substring(0, fileName.indexOf(FILE_KEYWORKS) + 5);
                if (logger.isDebugEnabled()) {
                    logger.debug("Load [error] error code configuration file [{}][{}]", resource.getFilename(), fileName);
                }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Load CLASSPATH root directory error code configuration file [{}]", resources.length);
            }
            for (Resource resource : resources) {
                String fileName = resource.getFilename();
                fileName = fileName.substring(0, fileName.indexOf(FILE_KEYWORKS) + 5);
                if (logger.isDebugEnabled()) {
                    logger.debug("Loading error code configuration file [{}][{}]", resource.getFilename(), fileName);
                }
